...
import EditExecutiveModal from "@/components/executive/EditExecuteModal";
import ViewExecutiveModal from "@/components/executive/ViewExecutiveModal";
...
const ExecutivePage = () => {
  ...
  const [openView, setOpenView] = useState<Executive | null>(null);
  const [openEdit, setOpenEdit] = useState<Executive | null>(null);
  const [vewDel, setViewDel] = useState<Executive | null>(null);
  const [showPassword, setShowPassword] = useState(false);
  const [openChangePass, setChangeOpenPass] = useState<Executive | null>(null);
  ...
  const safeRole = role as "super_admin" | "org_admin"
  ...
  const executives: Executive[] = Array.isArray(data) ? data : [];

  const filteredUsers = executives.filter((u) => {
    const matchesSearch = [
      u.firstName,
      u.lastName,
      u.email,
  ...
  return (
    <PrivateRoute allowedRoles={["super_admin", "org_admin"]}>
      <div className="space-y-8">
                ...
                {!isLoading &&
                  paginatedUsers.map((user) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 md:py-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-1 rounded-full text-sm font-medium bg-amber-100 text-amber-700`}
                        >
                          {user.role.replace("_", " ")}
                        </span>
                      </td>
        ...
        {openView && (
          <ViewExecutiveModal user={openView} onClose={() => setOpenView(null)} />
        )}
        {openCreate && (
          <CreateExecutive
